  ...
  useEffect(() => {
    const handleReturn = async () => {
      try {
        ...
        const response = await orderApi.momoReturn(params)
        setResult(response)
      } catch (err: any) {
        setError(err.message || 'Có lỗi xảy ra khi xử lý thanh toán MoMo')
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <PageWrapper>
        <div className="flex flex-col items-center justify-center py-24 space-y-4">
          ...
          <p className="text-lg font-black text-text-primary">Đang xử lý kết quả thanh toán MoMo...</p>
        </div>
      </PageWrapper>
    )
  }
        ...
        <Card className="p-8 text-center space-y-6 overflow-hidden relative">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-text-secondary font-medium px-4">
              {isSuccess 
                ? 'Cảm ơn bạn đã tin tưởng PharmaVN. Đơn hàng của bạn đã được thanh toán qua MoMo thành công.'
                : error || result?.message || 'Rất tiếc, đã có lỗi xảy ra trong quá trình thanh toán MoMo. Vui lòng thử lại.'}
            </p>
          </div>
